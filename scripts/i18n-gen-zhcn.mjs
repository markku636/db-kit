...
const GLOSSARY = {
  ...
  查詢: "查询", // OpenCC → 查找
  "未知的程序類型": "未知的例程类型", // 涵蓋程序 / 函式 / 觸發器，不單指預存程序
  預存程序: "存储过程", // OpenCC → 预存进程
  // 本專案的「程序」一律指 stored procedure（右鍵選單的 t("程序")、CLI 的「無法列出程序 / 函式」…）。
  // OpenCC 會轉成「进程」（OS process），意思完全跑掉。長詞優先排序讓「預存程序」與下面的
  // 「處理程序」先命中，所以這條只會落在真正單指預存程序的地方。
  程序: "存储过程", // OpenCC → 进程
  處理程序: "处理程序", // 這個「程序」才是 OS process，卡位以免被上面那條吃掉
