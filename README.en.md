...
- [x] **Internationalization (i18n)**: Traditional Chinese, Simplified Chinese, English, Japanese, and Korean — live switching without restart; frontend, Rust backend, and the `dbk` CLI all localized (`--lang`, `DBKIT_LANG`); each translation table is code-split so you only download your own. Japanese/Korean fall back to English (never Chinese) for any string not yet translated; the Simplified Chinese table is generated by `scripts/i18n-gen-zhcn.mjs` (OpenCC + a database-terminology glossary)
