...
| Streaming (Kafka) | Message browse + live tail, **JS filter expressions** (embedded boa) + deserializer choice (string / JSON / hex / Avro) + **search-more** scanning + JSON field projection, produce (headers / **Avro serialization** / **flow mode + random templates** / CSV bulk), **cluster overview** (brokers / URP health), topic config editing / add partitions / empty, full offset reset (with preview), consumer groups + lag, **monitoring & alerts** (background sampling / hand-rolled SVG charts / rules + OS notifications), **Schema Registry** (read-write / compatibility / version diff), **Protobuf decoding**, **Kafka Connect** (connector management / config validation), **ACL** management |
