...
| — | **SSH 終端機 + SFTP**（參考 Xshell）：側欄獨立「SSH 主機」清單（資料夾）、多分頁 xterm.js 終端機（密碼 / 私鑰 / keyboard-interactive / ssh-agent · Pageant；私鑰可匯入 OpenSSH、PuTTY PPK、PKCS#8、PEM（含 OpenSSL 3DES / AES 加密）、DER，支援 OpenSSH 使用者憑證，App 內金鑰庫可匯入 / 產生 / 匯出）、host key 首次 / 變更確認、命令列輸入條、SFTP 分割面板（瀏覽 / 上下傳 / 改名 / 刪除；Xftp 式 App 內編輯、權限、篩選；多選與資料夾批次上傳下載）、已設 SSH tunnel 的資料庫連線可直接開終端機；AI 只建議指令、由使用者按鈕送出（危險指令分級確認） | ✅ 完成 |
| — | 查詢效能分析（EXPLAIN） | ✅ 完成 |
