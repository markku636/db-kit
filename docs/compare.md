...
![整庫結構比對與同步腳本](./screenshots/10-schema-compare.png)

1. 在左側連線樹上，對著**來源資料庫**按右鍵 →「**結構比對…**」。
2. 對話框上方挑目標。預設是「**即時連線**」：
   - **連線** — 可以選同一條連線，也可以選**另一條連線**（只列出同類型且已連上的）。
   - **資料庫** — 目標資料庫。PostgreSQL 這裡顯示的是 **schema**。
   - 系統資料庫（`information_schema`、`master` 等）已經濾掉，不會出現在清單裡。
3. 左欄會列出來源的所有資料表，預設**全部勾選**。只想比其中幾張就先用「全不選」再挑，或用上方的搜尋框過濾。
4. 按右上角「**比對選取的 N 表**」。
5. 等進度跑完（會顯示「擷取結構中… 3/12」之類的字樣）。表很多時可以按「取消」中止。

比對結束後，左欄從「要比哪些表」換成「比出什麼」。
...
「直接執行」會先跳一個確認框，寫明「將在『prod / shop』執行 8 句語句（2 句為破壞性）」。**只要選取的語句裡有破壞性的，就必須再勾一次「我了解這會刪除目標的物件 / 資料，且無法復原。」** 才按得下去。
