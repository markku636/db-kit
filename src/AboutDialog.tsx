  ...
  return (
    <Modal open onClose={onClose} title={t("關於 {app}", { app: APP_NAME })} icon={Info} size="sm">
      <div className="flex flex-col items-center text-center gap-1 py-2">
        ...
        <img src="/apple-touch-icon.png" alt={APP_NAME} className="w-16 h-16 mb-2" draggable={false} />
        <div className="text-lg font-semibold">{APP_NAME}</div>
        <div className="flex items-center gap-1 text-xs text-fg/40 tabular-nums">
          <span>{t("版本")} {__APP_VERSION__}</span>
          <button
            type="button"
            onClick={() => copyToClipboard(`${APP_NAME} v${__APP_VERSION__} (${navigator.platform})`, t("已複製版本資訊"))}
        ...
        <p className="text-sm text-fg/60 mt-1">{t("跨資料庫管理工具：{dbs}", { dbs })}</p>
        ...
        <div className="mt-3 flex flex-col items-center gap-2 min-h-[52px]">
          ...
          {check.phase === "failed" && (
            <div className="text-sm text-fg/50">{t("檢查失敗（離線或已達 GitHub API 上限），稍後再試")}</div>
          )}
