...
export default function PubSubPanel({ connId, connName, onClose }: {
  ...
  const subscribe = async () => {
    ...
    if (channels.length === 0 && patterns.length === 0) {
      setErr(t("請至少輸入一個頻道或樣式（如 news 或 user:*）"));
      return;
    }
    setBusy(true);
    setErr(null);
  ...
  const publish = async () => {
    ...
    if (!pubChannel.trim()) { setErr(t("請輸入發佈頻道")); return; }
    setBusy(true);
    try {
      const n = await api.redisPublish(connId, pubChannel.trim(), pubMessage);
      toast.success(t("已發佈到 {trim}（{n} 個訂閱者）", { trim: pubChannel.trim(), n }));
      setPubMessage("");
    } catch (e: any) {
      setErr(e?.message ?? t("發佈失敗"));
    } finally {
      setBusy(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-app w-[760px] max-w-[94vw] h-[78vh] flex flex-col rounded-lg border border-fg/10 shadow-2xl"
        ...
        <div className="px-4 py-2 border-b border-fg/10 flex flex-wrap items-center gap-2 text-xs">
          <label className="text-fg/40">{t("頻道")}</label>
          <input value={channelsInput} onChange={(e) => setChannelsInput(e.target.value)}
            disabled={subscribed} placeholder={t("news, chat（空白 / 逗號分隔）")}
          ...
          <label className="text-fg/40">{t("樣式")}</label>
          ...
          {subscribed ? (
            <button type="button" onClick={unsubscribe} disabled={busy}
              className="px-3 py-1 rounded bg-amber-600/80 hover:bg-amber-600 text-fg disabled:opacity-40">{t("取消訂閱")}</button>
          ) : (
            <button type="button" onClick={subscribe} disabled={busy}
              className="px-3 py-1 rounded bg-accent text-white hover:bg-accent/90 disabled:opacity-40">{t("訂閱")}</button>
          )}
          <button type="button" onClick={() => setPaused((p) => !p)} title={t("暫停 / 繼續接收")}
            ...
            {paused ? <><Icon icon={Play} size={14} /> {t("繼續")}</> : <><Icon icon={Pause} size={14} /> {t("暫停")}</>}
          </button>
          <button type="button" onClick={() => setLines([])} title={t("清空訊息")}
            className="px-2 py-1 rounded border border-fg/15 hover:bg-fg/10 text-fg/60">{t("清空")}</button>
        </div>
        ...
        <div className="flex-1 overflow-auto p-3 mono text-xs leading-relaxed">
          {lines.length === 0 && (
            <div className="text-fg/30">{t("訂閱後，符合的訊息會即時顯示在這裡。")}</div>
          )}
          {lines.map((l, i) => (
