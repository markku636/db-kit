...
function StageNode({ node, depth }: { node: MongoPlanStage; depth: number }) {
  ...
  if (node.nReturned !== undefined) metrics.push(`回傳 ${node.nReturned}`);
  if (node.keysExamined !== undefined) metrics.push(`鍵掃描 ${node.keysExamined}`);
  if (node.docsExamined !== undefined) metrics.push(`文件掃描 ${node.docsExamined}`);
  ...
  return (
    <div style={{ marginLeft: depth * 16 }} className="space-y-1">
      <div className={`inline-flex flex-wrap items-center gap-2 rounded border px-2 py-1 text-xs ${stageColor(node)}`}>
        ...
        {node.stage === "COLLSCAN" && <span className="text-[10px]">全集合掃描</span>}
        {node.stage === "SORT" && <span className="text-[10px]">記憶體排序（無索引支撐）</span>}
    ...
    <div className="space-y-3 p-3 text-sm">
      ...
      {s ? (
        <div className="flex flex-wrap gap-2 text-xs">
          ...
          {s.executionTimeMillis !== null && (
            <span className="rounded bg-elevated border border-fg/10 px-2 py-1">
              耗時 <span className="mono font-semibold">{s.executionTimeMillis} ms</span>
            </span>
          )}
          {ratio !== null && (
            <span
              title="文件掃描 ÷ 回傳：>10 表示大量掃描才換到一筆結果，通常代表缺索引"
              className={`rounded border px-2 py-1 ${ratio > 10 ? "border-amber-300/40 bg-amber-500/10 text-amber-300" : "border-fg/10 bg-elevated"}`}
            >
              掃描比 <span className="mono font-semibold">{ratio.toFixed(1)}×</span>
            </span>
          )}
          {s.collscan ? (
        ...
        <div className="text-xs text-fg/45">
          verbosity=queryPlanner：僅計畫、未實際執行（無統計）。要看實際掃描數請改用 executionStats。
        </div>
      )}
